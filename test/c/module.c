...
{
    RECT* result = malloc(sizeof(RECT));
    result->x = x;
    result->y = y;
    return result;
}
...
short* check_short_ptr(short *x) { *x = *x + 1; return x; }
int* check_int_ptr(int *x) { *x = *x + 1; return x; }
long* check_long_ptr(long *x) { *x = *x + 1; return x; }
long long* check_long_long_ptr(long long *x) { *x = *x + 1; return x; }
float* check_float_ptr(float *x) { *x = *x + 1; return x; }
double* check_double_ptr(double *x) { *x = *x + 1; return x; }
unsigned short* check_unsigned_short_ptr(unsigned short *x) { *x = *x + 1; return x; }
unsigned int* check_unsigned_int_ptr(unsigned int *x) { *x = *x + 1; return x; }
unsigned long* check_unsigned_long_ptr(unsigned long *x) { *x = *x + 1; return x; }
unsigned long long* check_unsigned_long_long_ptr(unsigned long long *x) { *x = *x + 1; return x; }
char* check_char_ptr(char *x) { char* r = malloc(sizeof(char)*2); r = "!"; return r; }
void* check_void_ptr(void *x) { return x; }

short* check_const_short_ptr(const short *x) { short *r = malloc(sizeof(short)); *r = *x - 1; return r; }
int* check_const_int_ptr(const int *x) { int *r = malloc(sizeof(int)); *r = *x - 1; return r; }
long* check_const_long_ptr(const long *x) { long *r = malloc(sizeof(long)); *r = *x - 1; return r; }
long long* check_const_long_long_ptr(const long long *x) { long long *r = malloc(sizeof(long long)); *r = *x - 1; return r; }
float* check_const_float_ptr(const float *x) { float *r = malloc(sizeof(float)); *r = *x - 1; return r; }
double* check_const_double_ptr(const double *x) { double *r = malloc(sizeof(double)); *r = *x - 1; return r; }
unsigned short* check_const_unsigned_short_ptr(const unsigned short *x) { unsigned short *r = malloc(sizeof(short)); *r = *x - 1; return r; }
unsigned int* check_const_unsigned_int_ptr(const unsigned int *x) { unsigned int *r = malloc(sizeof(int)); *r = *x - 1; return r; }
unsigned long* check_const_unsigned_long_ptr(const unsigned long *x) { unsigned long *r = malloc(sizeof(long)); *r = *x - 1; return r; }
unsigned long long* check_const_unsigned_long_long_ptr(const unsigned long long *x) { unsigned long long *r = malloc(sizeof(long long)); *r = *x - 1; return r; }
...
// compile with: gcc -Wall -shared -fPIC -o libcmodule.so module.c
// copy to: /usr/lib/
